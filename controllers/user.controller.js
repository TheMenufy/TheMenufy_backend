...
return res.status(200).json({ message: 'Un e-mail a été envoyé', user: utilisateurPeuple });
  ...
  addRestaurantFranchise: async (req, res) => {
    try {
  
      const tokenLogin = req.body.tokenLogin;
      let decodeTokenLogin = jwt_decode(tokenLogin);
      let idUser = decodeTokenLogin.id;
      let role = decodeTokenLogin.role;
      ...
      if (idUser) {
        if (role === "resFranchise") {
          const { firstName, lastName, userName, phone, email, nameRes, address, cuisineType, taxeTPS, taxeTUQ, payCashMethod,franchiseFK ,color} = req.body;

          if (!firstName || !lastName || !userName || !email || !phone || !nameRes || !address || !cuisineType || !taxeTPS || !taxeTUQ || !franchiseFK ) {
            return res
              .status(400)
              .json({ message: "Not all fields have been entered" });
          }
            ...
            if (user) {
              ...
              const options = {
                ...
                html: `
                 ...
                 <h2 style="font-size: 18px; margin-bottom: 20px; text-align: center; text-align: center; color: #FA8072"> Welcome to Menu</h2>
                 
                 <p style="margin-top: 0; margin-bottom: 15px;">This is your work account credentials to sign in to the Menu application</p>
                 <a>Your email: ${email}</a>
                 <br>
                 <a>Your temporary password: ${pwdRandom}</a>
                 <p>Ordear Team</p>
                 </div>
          ...
          res.status(400).json({ message: "You are not authorized to add a new account" });
        }
      }
    } catch (error) {
      return res.status(500).json({ message: "Register failed: " + error.message });
