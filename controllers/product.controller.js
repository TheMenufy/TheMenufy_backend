...
addNew: async (req, res) => {
  ...
  updateById: async (req, res) => {
    try {
      const { name, description, price, disponibilityDuration, promotion, visibility, categoryFK,photo } =
        req.body;
console.log(req.body)
   ...
   //   product.gif_ = gif_;
      ...
      res.status(200).json({ saved, priceWithPromotion: priceWithPromotion });
    } catch (error) {
      res.status(500).json({ error: "Internal server error" });
    }
  },
